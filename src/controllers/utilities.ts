import { calc, HistoryItem, MemoryItem } from './Calculator.js';
...
function getStandardButtons(): ButtonConfig[] {
    return [
...
//sci button section
function getScientificButtons(): ButtonConfig[] {
    return [
...
export function renderButtons(): void {
    ...
    const grid = document.getElementById('btnGrid') as HTMLElement;

    const newGrid = grid.cloneNode(false) as HTMLElement;
    ...
    newGrid.innerHTML = buttons.map(btn => {
        ...
        const label =
            calc.secondMode && btn.altLabel
                ? btn.altLabel
                : btn.label;

        const secondActiveClass =
            btn.action === 'second' && calc.secondMode
                ? 'btn-second-active'
                : '';
...
export function updateDisplay(): void {
    ...
    const displayEl = document.getElementById('displayValue') as HTMLElement;
    const exprEl    = document.getElementById('displayExpression') as HTMLElement;
    ...
    if (val.length > 16) displayEl.classList.add('xsmall-text');
    else if (val.length > 10) displayEl.classList.add('small-text');
}


//History Renderer
export function addToHistory(expr: string, result: string): void {
    calc.historyList.unshift({ expr, result });
    if (calc.currentTab === 'history') renderHistoryPanel();
}

export function renderHistoryPanel(): void {

    const content = document.getElementById('historyContent') as HTMLElement;
    ...
    if (calc.currentTab === 'history') {
        ...
        if (calc.historyList.length === 0) {
            ...
            content.innerHTML = calc.historyList.map((item: HistoryItem, i: number) => `
            ...
            content.querySelectorAll('.history-item').forEach(el => {
                el.addEventListener('click', () => {
                    ...
                    calc.display = calc.historyList[idx]?.result ?? '0';
        ...
        if (calc.memoryList.length === 0) {
            ...
            content.innerHTML = calc.memoryList.map((item: MemoryItem) => `
...
export function resetCalculatorState(): void {
